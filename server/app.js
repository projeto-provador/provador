...
import { pool, ensureSchemaOnce, insertLead } from './db.js';
...
// Garante o schema uma vez por processo antes das rotas de dados.
// Em serverless (Vercel) roda no primeiro request após um cold start.
// Desative com AUTO_INIT_SCHEMA=false se preferir migrar manualmente.
app.use(async (req, res, next) => {
  if (!req.path.startsWith('/api/')) return next();
  if (String(process.env.AUTO_INIT_SCHEMA).toLowerCase() === 'false') return next();
  try {
...
// Front-end estático (usado no modo servidor tradicional / local; no Vercel os
// estáticos de public/ são servidos direto pela plataforma).
...
app.post('/api/leads', async (req, res) => {
  ...
  // Em serverless a função pode ser congelada após a resposta, então aguardamos
  // as notificações antes de responder. São best-effort e não derrubam o lead.
